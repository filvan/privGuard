...
from src.examples.program.libretaxi.repository import libretaxi_userIdsAround, libretaxi_saveUser, libretaxi_savePost, \
    libretaxi_userPostedRecently
...
def informUsersAround(data_folder, **kwargs):
    ...
    if user.ShadowBanned != "Y":
        ...
        msg = textWithContacts + user.DataSubjectID
        ParseMode = ""
        if len(user.Username) == 0:
            # sets for message a parsingMode based on the username
            ParseMode = "MarkdownV2"
    ...
    users_around = libretaxi_userIdsAround.run(data_folder, **kwargs)
...
def run(data_folder, **kwargs):
    ...
    text = kwargs.get('extra_args').get('text')
    if libretaxi_userPostedRecently.run(data_folder, **kwargs):
        msg = locales.post_menu_wait + user_infos['DataSubjectID']
        ...
        msg_driver = locales.post_menu_driver_example + user_infos['DataSubjectID']
        msg_passenger = locales.post_menu_passenger_example + user_infos['DataSubjectID']
        return user_infos
    else:
        errors = libretaxi_validation.run(text, locales)
        if len(errors) > 0:
            msg = user_infos['DataSubjectID'] + errors + " " + locales.post_menu_or + "/cancel"
            return user_infos
        ...
        informUsersAround(data_folder, **kwargs)
        # informUsersAround(user_infos['Longitude'], user_infos['Latitude'], cleanText, results, user_infos)

        msg = user_infos['DataSubjectID'] + locales.post_menu_sent
